...
```
setter方法可以检验输入的合法性：
在这个例子中，如果在创建 `Person` 对象时提供的 `age` 参数不合法，那么就会抛出一个 `ValueError` 异常。但是，如果在创建对象后需要修改 `age` 属性的值，那么这种验证就无法进行了。
如果你使用 setter 方法，那么无论何时修改属性的值，都会进行验证。这就是使用 setter 方法的一个优点。
```Python
class Person(object):
